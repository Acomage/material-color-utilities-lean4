...
def _linrgbFromScaledDiscount :=
  #v[#v[1373.2198709594231, -1100.4251190754821, -7.278681089101213],
    #v[-271.815969077903, 559.6580465940733, -32.46047482791194],
    #v[1.9622899599665666, -57.173814538844006, 308.7233197812385]]

def _yFromLinrgb := #v[0.2126, 0.7152, 0.0722]

set_option maxRecDepth 523

def _criticalPlanes :=
  #v[0.015176349177441876,
...
def _areInCyclicOrder (a b c : Float) : Bool :=
  let deltaAB := _sanitizeRadians (b - a)
  let deltaAC := _sanitizeRadians (c - a)
  deltaAB < deltaAC

def _intercept (source mid target : Float) : Float :=
  (mid - source) / (target - source)

def _lerpPoint (source target : Vector Float 3) (t : Float) : Vector Float 3 :=
  #v[source[0] + (target[0] - source[0]) * t,
    source[1] + (target[1] - source[1]) * t,
    source[2] + (target[2] - source[2]) * t]

def _setCoordinate (source target : Vector Float 3) (coordinate : Float) (axis : Nat) (h : axis ∈ [:3]) : Vector Float 3 :=
  let t := _intercept source[axis] coordinate target[axis]
  _lerpPoint source target t

def _isBounded (x : Float) : Bool :=
  0.0 <= x && x <= 100.0

def _nthVertex (y : Float) (n : Int) : Vector Float 3 :=
  let kR := _yFromLinrgb[0]
  let kG := _yFromLinrgb[1]
  let kB := _yFromLinrgb[2]
  let coordA := if n % 4 <= 1 then 0.0 else 100.0
  let coordB := if n % 2 == 0 then 0.0 else 100.0
  if n < 4 then
    let g := coordA
    let b := coordB
    let r := (y - kG * g - kB * b) / kR
    if _isBounded r then
      #v[r, g, b]
    else
      #v[-1.0, -1.0, -1.0]
  else if n < 8 then
    ...
    let r := coordA
    let g := coordB
    let b := (y - kR * r - kG * g) / kB
    if _isBounded b then
      #v[r, g, b]
    else
      #v[-1.0, -1.0, -1.0]

def _bisectToSegment (y targetHue : Float) : Vector (Vector Float 3) 2 := Id.run do
  let mut left := #v[-1.0, -1.0, -1.0]
  let mut right := #v[-1.0, -1.0, -1.0]
  let mut leftHue := 0.0
  let mut rightHue := 0.0
  let mut initialized := false
  let mut uncut := true
  for n in [:12] do
    let mid := _nthVertex y n
    if mid[0] < 0.0 then
      continue
    let midHue := _hueOf mid
    if !initialized then
      left := mid
      right := mid
      leftHue := midHue
      ...
      initialized := true
      continue
    if uncut || _areInCyclicOrder leftHue midHue rightHue then
      uncut := false
      if _areInCyclicOrder leftHue targetHue midHue then
        right := mid
        rightHue := midHue
      else
        left := mid
...
def _midpoint (a b : Vector Float 3) : Vector Float 3 :=
  #v[(a[0] + b[0]) / 2.0, (a[1] + b[1]) / 2.0, (a[2] + b[2]) / 2.0]

def _criticalPlaneBelow (x : Float) : Int :=
  (x - 0.5).floor.toInt64.toInt

def _criticalPlaneAbove (x : Float) : Int :=
  (x + 0.5).ceil.toInt64.toInt

def _bisectToLimit (y targetHue : Float) : Vector Float 3 := Id.run do
  let segment := _bisectToSegment y targetHue
  let mut left := segment[0]
  let mut leftHue := _hueOf left
  ...
  for h:axis in [:3] do
    if left[axis] != right[axis] then
      ...
      if left[axis] < right[axis] then
        ...
        lPlane := _criticalPlaneBelow (_trueDelinearized right[axis])
        rPlane := _criticalPlaneAbove (_trueDelinearized left[axis])
      for i in [:8] do
        if (rPlane - lPlane).natAbs <= 1 then
          break
        else
          let mPlane := ((lPlane + rPlane) / 2).toNat
          have h' : mPlane < 255 := by
            sorry
          let midPlaneCoordinate : Float := _criticalPlanes[mPlane]
          let mid := _setCoordinate left right midPlaneCoordinate axis h
          let midHue := _hueOf mid
          if _areInCyclicOrder leftHue targetHue midHue then
            right := mid
            rPlane := mPlane
...
def _findResultByJ (hueRadians chroma y : Float) : Int := Id.run do
  ...
  let hSin := Float.sin (hueRadians)
  let hCos := Float.cos (hueRadians)
  for iterationRound in [:5] do
    let jNormalized := j / 100.0
    let alpha := if chroma == 0.0 || j == 0.0 then 0.0 else chroma / jNormalized.sqrt
    let t := (alpha * tInnerCoeff)^(1.0/0.9)

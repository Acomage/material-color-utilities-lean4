...
def fromXyzInViewingConditions (x y z : Float) (viewingconditions : ViewingConditions) : Cam16 :=
  ...
  let rA := Float.ofInt (MathUtils.signum rD) * 400.0 * rAF / (rAF + 27.13)
  let gA := Float.ofInt (MathUtils.signum gD) * 400.0 * gAF / (gAF + 27.13)
  let bA := Float.ofInt (MathUtils.signum bD) * 400.0 * bAF / (bAF + 27.13)
  ...
  let atanDegrees := atan2 * 180.0 / MathUtils.pi
  let hue := if atanDegrees < 0.0
    then atanDegrees + 360.0
    else if atanDegrees >= 360.0
      then atanDegrees - 360.0
      else atanDegrees
  let hueRadians := hue * MathUtils.pi / 180.0
  ...
  let eHue := 0.25 * (Float.cos (huePrime * MathUtils.pi / 180.0 + 2.0) + 3.8)
...
def fromIntInViewingConditions (argb : Int) (viewingconditions : ViewingConditions) : Cam16 :=
...
def fromJchInViewingConditions (J C h : Float) (viewingconditions : ViewingConditions) : Cam16 :=
  ...
  let hueRadians := h * MathUtils.pi / 180.0
  let jstar := (1.0 + 100.0 * 0.007) * J / (1.0 + 0.007 * J)
...
def fromUcsInViewingConditions (jstar astar bstar : Float) (viewingconditions : ViewingConditions) : Cam16 :=
  ...
  let h := Float.atan2 b a * (180.0 / MathUtils.pi)
...
def fromUcs (jstar astar bstar : Float) : Cam16 :=
  fromUcsInViewingConditions jstar astar bstar (ViewingConditions.standard)

def xyzInViewingConditions (self : Cam16) (viewingconditions : ViewingConditions) : Vector Float 3 :=
  ...
  let hRad := self.hue * MathUtils.pi / 180.0
  ...
  let rCBase := MathUtils.max 0.0 (27.13 * rA.abs / (400.0 - rA.abs))
  let rC := Float.ofInt (MathUtils.signum rA) * (100.0 / viewingconditions.fl) * (rCBase^(1.0 / 0.42))
  let gCBase := MathUtils.max 0.0 (27.13 * gA.abs / (400.0 - gA.abs))
  let gC := Float.ofInt (MathUtils.signum gA) * (100.0 / viewingconditions.fl) * (gCBase^(1.0 / 0.42))
  let bCBase := MathUtils.max 0.0 (27.13 * bA.abs / (400.0 - bA.abs))
  let bC := Float.ofInt (MathUtils.signum bA) * (100.0 / viewingconditions.fl) * (bCBase^(1.0 / 0.42))
...
def viewed (self : Cam16) (viewingconditions : ViewingConditions) : Int :=
...
def toInt (self : Cam16): Unit -> Int :=
  fun () => viewed self ViewingConditions.sRgb
